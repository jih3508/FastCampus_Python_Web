...
# DB 생성 & Auto Commit
# 본인 DB 경로
conn = sqlite3.connect("./section13/resource/records.db", isolation_level=None)
...
with open("./section13/resource/word.txt", 'r') as f:
    for c in f:
...
while n <= 5:
    ...
    if str(q).strip() == str(x).strip(): # 입력 확인(공백 제거)
        print("Pass!")
        # 정답 소리 재생
        winsound.PlaySound("./section13./sound/good.wav", winsound.SND_FILENAME)
        cor_cnt += 1
    else:
        # 오답 소리 재생
        print("Wrong!")
        winsound.PlaySound("./section13./sound/bad.wav", winsound.SND_FILENAME)
